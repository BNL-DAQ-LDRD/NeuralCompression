...
class BCAELossMetrics(nn.Module):
    """
    ...
    def update(self, metrics):
        ...
        Output:
        """
        loss_avg_reg = torch.mean(torch.Tensor(metrics['reg. loss']))
        loss_avg_clf = torch.mean(torch.Tensor(metrics['clf. loss']))
        self.clf_lambda = (loss_avg_reg / loss_avg_clf).item()
