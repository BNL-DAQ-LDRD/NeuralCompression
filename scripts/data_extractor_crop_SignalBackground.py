        ...
        for i, file_name in enumerate(h5_file_list):
            ...
            with h5py.File(str(file_name), 'r') as fp:
                for s in range(sample_dim):
                    # buffer and load the layers
                    buf_adc = np.zeros((azimuthal_dim, z_dim, layer_dim), dtype='uint16')
                    buf_sbg = np.zeros((azimuthal_dim, z_dim, layer_dim), dtype='uint8')
                    for l in range(layer_dim):
                        key_adc = f'/TimeFrame_{s}/Data_Layer{layer_start_idx + l}'
                        key_sbg = f'/TimeFrame_{s}/Data_Layer_SignalBackground{layer_start_idx + l}'
                        buf_adc[:, :, l] = fp[key_adc][()]
                        buf_sbg[:, :, l] = fp[key_sbg][()]
